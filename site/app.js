...
var CharacterError = class extends Error {
  constructor() {
    super(...arguments);
    __publicField(this, "name", "CharacterError");
  }
};
...
function validateOptions(frameIndex, options) {
...
function calculateActivityMetrics(calendar2) {
  const days = flattenDays(calendar2);
  const currentStreak = countCurrentStreak(days);
  const displayedTotal = days.reduce((total, day) => total + day.count, 0);
  const endYear = Number(calendar2.endDate.slice(0, 4));
...
function flattenDays(calendar2) {
  return calendar2.weeks.flatMap((week) => [...week]);
}
...
function planScene(calendar2, config) {
  const experience = config.experience ?? createDefaultExperience();
  const metrics = calculateActivityMetrics(calendar2);
  const seed = hashSeed([
  ...
  return Object.freeze({
    season: seasonForDate(calendar2.endDate, experience.calendar.hemisphere),
    weather,
    vignette,
    metrics,
...
var HEX_COLOR_PATTERN2 = /^#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i;
var MAX_COLORS = 256;
var Palette = class {
  constructor(background) {
    __publicField(this, "colors", []);
    __publicField(this, "indices", /* @__PURE__ */ new Map());
...
function normalizeHex(value) {
  if (!HEX_COLOR_PATTERN2.test(value)) {
    throw new Error(`invalid six-digit hex color '${value}'`);
  }
  return value.toUpperCase();
}
function parseHex(value) {
  const match = HEX_COLOR_PATTERN2.exec(value);
  if (match === null) {
    throw new Error(`invalid six-digit hex color '${value}'`);
  }
...
function createScene(config, character) {
  ...
  return Object.freeze({
    layout,
    render(calendar2, frameIndex) {
      validateCalendar(calendar2);
      const plan = planScene(calendar2, config);
...
function drawCalendar(buffer, palette, config, layout, calendar2, wakeColumn) {
  ...
  for (let column = 0; column < layout.columns; column += 1) {
    const week = calendar2.weeks[column];
    if (week === void 0) {
...
function validateCalendar(calendar2) {
  if (calendar2.weeks.length !== GRID_COLUMNS) {
    throw new Error(`calendar must contain exactly ${GRID_COLUMNS} weeks`);
  }
  if (!calendar2.weeks.every((week) => week.length === GRID_ROWS)) {
...
// src/customizer.ts
var DEFAULT_CUSTOMIZER_URL = "https://aldeniaalexandra.github.io/ube/";
var fixture = calendar_default;
var calendar = normalizeCalendar(fixture.days, fixture.endDate);
var defaultCharacter = validateCharacter(ube_default);
var state;
var animationHandle = 0;
function bootCustomizer() {
  document.title = "Ube Moonlit Garden customizer";
  const canvas = getElement("preview");
...
function readConfig() {
  ...
  const targetBytes = readNumber("target-bytes", 1e5, 5e6);
  const hardMaxBytes = readNumber("hard-max-bytes", targetBytes, 5e6);
...
function readExperience() {
  ...
  const identityEnabled = getElement("identity-enabled").checked;
  ...
  return {
    ...
    calendar: {
      ...
      hemisphere: getInput("hemisphere").value
    },
    stats: {
      period: getInput("stats-period").value,
      showStreak: getElement("show-streak").checked,
      showTotal: getElement("show-total").checked
    },
    identity: {
      enabled: identityEnabled,
      name: getInput("identity-name").value.trim(),
      role: getInput("identity-role").value.trim(),
      style: getInput("identity-style").value
    },
    link
  };
}
...
function recolorCharacter(character, accent) {
  const palette = { ...character.palette, P: accent };
  return { ...character, palette };
}
function animate(timestamp) {
  if (state !== void 0) {
    const millisecondsPerFrame = 1e3 / state.config.output.fps;
...
function renderFrame() {
  ...
  for (let index = 0; index < frame.pixels.length; index += 1) {
    const color = frame.palette[frame.pixels[index]] ?? [0, 0, 0];
...
function stripResolvedFields(config) {
...
function createCustomizerLink(config) {
...
function syncIdentityVisibility() {
  const enabled = getElement("identity-enabled").checked;
